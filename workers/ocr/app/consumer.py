...
def run_consumer(
    *,
    ...
    publish_next: Callable[[BlockingChannel, dict, dict], None] | None = None,
) -> None:
    connection = _connect()
    ...
    def on_message(ch: BlockingChannel, method, properties, body: bytes) -> None:
        ...
        try:
            ...
            LOG.info('message.received', extra={
                ...
                'message_id': message_id, 'document_id': document_id, 'attempt': attempt,
            })

            result = handler(envelope, ch)

            if publish_next and result and result.get('publish_next'):
                publish_next(ch, envelope, result['publish_next'])

            ch.basic_ack(delivery_tag=method.delivery_tag)
            PROCESSED.labels(stage=stage).inc()
            LOG.info('message.acked', extra={
                'event': 'message.acked', 'stage': stage, 'message_id': message_id,
            })
        except PermanentError as e:
            FAILED.labels(stage=stage, reason='permanent').inc()
            LOG.error('message.failed.permanent', extra={
                'event': 'message.failed.permanent', 'stage': stage,
                'reason': str(e), 'envelope': envelope,
            })
            ch.basic_ack(delivery_tag=method.delivery_tag)
            ...
            LOG.exception('message.failed.unhandled', extra={
                'event': 'message.failed.unhandled', 'stage': stage,
                'error': str(e),
            })
            try:
